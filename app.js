...
function confettiBurst(){
  const colors = ['#00f5d4','#ff2fd0','#b967ff','#c6ff3d','#ff6b4a'];
  for(let i=0;i<26;i++){
...
window.addEventListener('wheel', (e)=>{
  ...
  const nearTop = window.scrollY < 60;
  if(e.deltaY>60 && nearBottom && flowOrder[id] && !flowScrollLock){
    flowScrollLock = true;
    flowAdvance(id, 'next');
    setTimeout(()=>{ flowScrollLock=false; }, 900);
  } else if(e.deltaY<-60 && nearTop && flowOrderReverse[id] && !flowScrollLock){
    flowScrollLock = true;
    flowAdvance(id, 'prev');
    setTimeout(()=>{ flowScrollLock=false; }, 900);
  }
