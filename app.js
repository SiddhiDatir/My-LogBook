...
async function startQuiz(){
  ...
  try{
    ...
    const pickedCat = gkCategories[Math.floor(Math.random()*gkCategories.length)];
    const resp = await fetch(`https://opentdb.com/api.php?amount=8&type=multiple&category=${pickedCat}`, {signal:controller.signal});
