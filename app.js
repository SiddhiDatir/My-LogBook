...
function addOfficeTask(){
  ...
  DATA.officeTasks.push({id:uid(), text, domain:document.getElementById('officeDomain').value, kind:document.getElementById('officeKind').value, date:d, done:false, note:''});
  inp.value=''; persist(); renderOffice();
  mascotReact('happy', 'Logged in Office.');
}
function toggleOffice(id){
  const o = DATA.officeTasks.find(x=>x.id===id); if(o){ o.done=!o.done; persist(); renderOffice(); }
}
...
function renderOffice(){
  ...
  list.innerHTML = todays.map(o=>`
    <li class="${o.done?'done':''}">
      ...
      <span style="font-size:10px;color:var(--muted);font-family:var(--mono);">${o.domain}${o.kind==='learning'?' · learning':''}</span>
