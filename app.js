...
function load(){
  ...
  return {
    ...
    gameProgress: {quizUsed:[], scrambleUsed:[]},
...
function renderSnapshot(){
  ...
  const officeToday = DATA.officeTasks.filter(o=>o.date===d);
  document.getElementById('snapOfficeEmpty').style.display = officeToday.length? 'none':'block';
  document.getElementById('snapOffice').innerHTML = officeToday.map(o=>`
    <li class="${o.done?'done':''}"><input type="checkbox" ${o.done?'checked':''} onchange="toggleOffice(${o.id})">
    <span class="txt">${escapeHtml(o.text)}</span>
    <span style="font-size:10px;color:var(--muted);font-family:var(--mono);">${o.domain}</span></li>`).join('');

  const atRisk = DATA.habits.filter(h=>!h.log[d]);
  countUpEl(document.getElementById('snapHabitRisk'), atRisk.length);
  if(open.length>=6){
    mascotReact('worried', `${open.length} things stacking up.`, 2600);
  }
...
function addShopItem(){
  ...
  if(!inp.value.trim()) return;
  DATA.shopping.push({id:uid(), text:inp.value.trim(), cat:document.getElementById('shopCat').value, done:false});
  inp.value=''; persist(); renderShopping();
}
function toggleShop(id){ const s=DATA.shopping.find(x=>x.id===id); if(s){ s.done=!s.done; persist(); renderShopping(); } }
function delShop(id){ DATA.shopping = DATA.shopping.filter(x=>x.id!==id); persist(); renderShopping(); }
function clearCheckedShop(){ DATA.shopping = DATA.shopping.filter(x=>!x.done); persist(); renderShopping(); }
...
function addOfficeTask(redoOf){
  ...
  DATA.officeTasks.push({
    id:uid(), text,
    domain:document.getElementById('officeDomain').value,
    kind:document.getElementById('officeKind').value,
    date:d, dueDate: dueInput || d,
    done:false, note:'', completedDate:null,
    redoOf: redoOf || null
  });
  inp.value=''; document.getElementById('officeDueDate').value='';
  persist(); renderOffice();
...
function addOfficeNote(id){
  ...
  if(note!==null){ o.note = note.trim(); persist(); renderOffice(); }
}
function extendOfficeDue(id){
  const o = DATA.officeTasks.find(x=>x.id===id);
  if(!o) return;
  const newDate = prompt('New due date (YYYY-MM-DD)', o.dueDate || o.date);
  if(newDate){ o.dueDate = newDate.trim(); persist(); renderOffice(); }
}
...
function delOffice(id){ DATA.officeTasks = DATA.officeTasks.filter(x=>x.id!==id); persist(); renderOffice(); }
...
async function startQuiz(){
  ...
  try{
    ...
    const resp = await fetch('https://opentdb.com/api.php?amount=8&type=multiple', {signal:controller.signal});
...
function renderQuizStage(){
  ...
  if(quizIndex >= quizOrder.length){
    stage.innerHTML = `<div style="font-size:16px;font-weight:700;">Round done — ${quizScore}/${quizOrder.length} correct</div>`;
    scoreEl.textContent = '';
  ...
  quizAnswered = false;
  stage.innerHTML = `
    <div style="font-size:10.5px; color:var(--muted); margin-bottom:6px; text-transform:uppercase; letter-spacing:.5px;">${quizMode==='online'?'● live — fresh from the internet':'○ offline bank — no internet detected'}</div>
...
function renderAll(){
  ...
  renderOffice(); renderNotes(); renderStudy(); renderRoutine();
}
