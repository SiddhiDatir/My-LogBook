...
function addVisionItem(){
  ...
  const img = document.getElementById('visionImg').value.trim();
  if(!text && !img) return;
  DATA.vision.push({id:uid(), text, img});
  document.getElementById('visionText').value=''; document.getElementById('visionImg').value='';
...
function renderVision(){
  ...
  grid.innerHTML = DATA.vision.map((v,i)=>`
    <div class="vision-card" style="${v.img?'':'background:'+visionColors[i%visionColors.length]+';'}">
      ${v.img? `<img src="${escapeHtml(v.img)}" onerror="this.style.display='none'">` : ''}
...
function addOfficeTask(){
  ...
  if(!inp.value.trim()) return;
  DATA.officeTasks.push({id:uid(), text:inp.value.trim(), domain:document.getElementById('officeDomain').value, date:todayStr(), done:false});
...
function renderOffice(){
  ...
  list.innerHTML = todays.map(o=>`
    <li class="${o.done?'done':''}">
      ...
      <span style="font-size:10px;color:var(--muted);font-family:var(--mono);">${o.domain}</span>
