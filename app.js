...
function dedupeAllTasks(){
  const seen = new Set();
  DATA.allTasks = (DATA.allTasks||[]).filter(t=>{
    const key = [t.category, t.title, t.dueDate, t.status].join('|');
...
function toggleTheme(){
  ...
  localStorage.setItem('lifeos_theme', isLight ? 'light' : 'dark');
}
...
function confettiBurst(){
  const colors = ['#9b7bff','#5eb8ff','#8be08b','#c9b6ff'];
  for(let i=0;i<26;i++){
...
function renderHabits(){
  ...
  const catColors = ['var(--teal)','var(--amber)','var(--violet)','var(--red)'];
  ...
  if(container){
    container.innerHTML = cats.map((cat,ci)=>{
      ...
      return `<div class="card" style="margin-bottom:14px;">
        <h3 style="color:${catColors[ci%catColors.length]};">${escapeHtml(cat)}</h3>
        ${items.map(h=>{
...
// ---------- daily routine — now just a quick-glance view of your Daily-frequency habits ----------
// (backed by DATA.allHabits, so it's editable, deletable, and persists/syncs like everything else)
function addRoutineItem(){
  const inp = document.getElementById('routineInput');
  if(!inp.value.trim()) return;
  DATA.allHabits.push({ id:uid(), name:inp.value.trim(), category:'General', frequency:'daily', log:{} });
  inp.value=''; persist(); renderRoutine(); renderHabits(); renderSnapshot();
}
function toggleRoutine(id){ toggleHabitToday(id); renderRoutine(); }
function delRoutineItem(id){ delHabit(id); renderRoutine(); }
function renderRoutine(){
  const list = document.getElementById('routineList');
  if(!list) return;
  const daily = DATA.allHabits.filter(h=>h.frequency==='daily');
  const d = todayStr();
  list.innerHTML = daily.map(h=>`
    <li class="${h.log[d]?'done':''}">
      <input type="checkbox" ${h.log[d]?'checked':''} onchange="toggleRoutine(${h.id})">
      <span class="txt">${escapeHtml(h.name)}</span>
      <span class="del" style="cursor:pointer;color:var(--muted);" onclick="delRoutineItem(${h.id})">✕</span>
    </li>`).join('');
}
...
const flowOrderReverse = Object.fromEntries(Object.entries(flowOrder).map(([from,to])=>[to,from]));
...
function flowAdvance(fromView, dir){
  const map = dir==='prev' ? flowOrderReverse : flowOrder;
  const next = map[fromView];
  if(!next) return;
...
function initFlowNext(){
  ...
  document.querySelectorAll('.flow-prev[data-flow]').forEach(el=>{
    const from = el.dataset.flow;
    const prev = flowOrderReverse[from];
    const prevTab = document.querySelector(`.tab[data-view="${prev}"]`);
    if(prevTab){
      const label = prevTab.textContent.replace(/^\s*\d+\s*/,'').trim();
      const span = el.querySelector('span');
      if(span) span.textContent = label;
    }
  });
...
window.addEventListener('scroll', ()=>{
  ...
  if(!activeView){ lastScrollY = window.scrollY; return; }
  const id = activeView.id.replace('view-','');
  const y = window.scrollY;
  const goingDown = y > lastScrollY;
  const goingUp = y < lastScrollY;
  const nearBottom = (document.body.scrollHeight - y - window.innerHeight) < 60;
  const nearTop = y < 60;
  if(goingDown && nearBottom && flowOrder[id] && !flowScrollLock){
    flowScrollLock = true;
    flowAdvance(id, 'next');
    setTimeout(()=>{ flowScrollLock=false; }, 900);
  } else if(goingUp && nearTop && flowOrderReverse[id] && !flowScrollLock){
    flowScrollLock = true;
    flowAdvance(id, 'prev');
    setTimeout(()=>{ flowScrollLock=false; }, 900);
  }
...
setTimeout(()=>{
  const activeView = document.querySelector('.view.active');
  if(activeView && activeView.getBoundingClientRect().bottom > window.innerHeight + 40){
    mascotReact('happy', 'Scroll down for today\'s snapshot ↓', 3200);
  }
}, 900);
